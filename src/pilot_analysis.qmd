...
pacman::p_load(tidyverse, here, readxl, estimatr, modelsummary, effectsize)
```
...
```{r}
"ethnic_linked_fate", "racial_linked_fate", "asian_interests"
```
...
```{r}
mod_main <- tidy(standardize(lm(china_virus_racist ~ factor(chinese) + important_democracy_scale + china_fav + china_threat + china_demo + american_id_importance + factor(dem) + factor(gop) + ethnic_linked_fate + racial_linked_fate + asian_interests + violence_asian_cat + factor(racial_covid_dis_cat), data = df)), conf.int = T) %>%
