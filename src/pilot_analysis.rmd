...
- `Asian ID importance`: How important is being Asian or Asian American to your identity? (1-4 scale)

- `Asian common interests`: To what extent do you agree or disagree with the following statement?: “Different groups of Asian Americans share common interests and goals and should work together to achieve them.” (1-4 scale)
...
**Confouunders / Moderating variables** 
...
Table \@ref(tab:main-chinese) presents the estimated associations between the "Chinese virus racist" variable and predictors, while Table \@ref(tab:main-table) presents the estimated associations between all five outcome variables and predictors.

```{r, echo = FALSE}
mods_cv <- list(
  "Model 1" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance, df)),
  "Model 2" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + china_fav + china_threat + china_demo, df)),
  "Model 3" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + pol_strong_leader + pol_democratic + pol_army_rule + china_fav + china_threat + china_demo, df)),
  "Model 4" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + pol_strong_leader + pol_democratic + pol_army_rule + china_fav + china_threat + china_demo + dem + gop, df)),
  "Model 5" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + pol_strong_leader + pol_democratic + pol_army_rule + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop, df)),
  "Model 6" = standardize(lm(china_virus_racist ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + pol_strong_leader + pol_democratic + pol_army_rule + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop + american_id_importance + national_attachment + chinese, df))
)
```
...
mods <- list(
  ...
  "Asian ID importance" = standardize(lm(asian_id_importance ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop + american_id_importance + national_attachment + chinese, df)),
  "Asian common interests" = standardize(lm(asian_interests ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop + american_id_importance + national_attachment + chinese, df))
  )
```
...
```{r asian-id-importance, echo = FALSE}
df %>%
  mutate(asian_nat) %>%
  nest_by(asian_nat) %>%
    mutate(models = list(standardize(lm(asian_id_importance ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop + american_id_importance + national_attachment, data)))) %>%
    pull(models, name = asian_nat) %>%
    modelsummary(
      ., 
      title = "Asian ID importance",
      coef_omit = "Intercept",
      fmt = 2,
      estimate  = "{estimate}{stars}",
      statistic = "({p.value})",
...
```{r asian-common-interests, echo = FALSE}
df %>%
  mutate(asian_nat) %>%
  nest_by(asian_nat) %>% 
    mutate(models = list(standardize(lm(asian_interests ~ violence_asian_cat + racial_covid_dis_cat + democracy_importance + china_fav + china_threat + china_demo + female + age + us_born_cat + dem + gop + american_id_importance + national_attachment, data)))) %>%
    pull(models, name = asian_nat) %>%
    modelsummary(
      ., 
      title = "Asian common interests",
      coef_omit = "Intercept",
      fmt = 2,
      estimate  = "{estimate}{stars}",
      statistic = "({p.value})",
