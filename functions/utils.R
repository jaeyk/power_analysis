...
get_db_rl_estimates <- function(treated_n) {
  ...
  basic_config() %>%
    ...
    # discrete
    add_moderator("Stratified", chinese_origin, democracy_cat, china_threatend_high, DEM, GOP) %>%
    # continuous
  ...
  df_sub_splits %>%
    estimate_QoI(chinese_origin, democracy_cat, china_threatend_high, DEM, GOP) -> results
